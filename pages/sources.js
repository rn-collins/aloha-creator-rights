...
export default function Sources(){
  return <main style={{maxWidth:880,margin:'0 auto',padding:'64px 24px',fontFamily:'Fira Sans,system-ui',lineHeight:1.65,color:'#111214'}}>
    ...
    <a href="/" style={{color:'#12379B'}}>← Creator Rights Issue Spotter</a>
    <p style={{fontSize:12,letterSpacing:'.1em',textTransform:'uppercase',color:'#4E4C4A',marginTop:40}}>Authority register · verified {VERIFIED_AT} · next review {NEXT_REVIEW}</p>
    ...
    <div style={{background:'#FFFFFF',border:'1px solid #C9C6BF',padding:'14px 18px',marginBottom:34,fontSize:13,color:'#4E4C4A'}}>Automated currency rule: an authority becomes <strong>review due</strong> after {NEXT_REVIEW}. “Current” means only that the linked official source was checked on {VERIFIED_AT}; it does not guarantee no later amendment, decision, enforcement action, or jurisdiction-specific rule exists.</div>
    {sources.map(s=>{const state=authorityState(s);return <section key={s.id} id={s.id} style={{borderTop:'1px solid #C9C6BF',padding:'26px 0'}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:14,alignItems:'flex-start',flexWrap:'wrap'}}>
        <div style={{fontSize:12,color:'#12379B',fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em'}}>{s.topic}</div>
      ...
      <h2 style={{fontSize:21,margin:'7px 0'}}><a href={s.url} target="_blank" rel="noopener noreferrer" style={{color:'#111214'}}>{s.title} ↗</a></h2>
      <dl style={{display:'grid',gridTemplateColumns:'minmax(120px,170px) 1fr',gap:'6px 14px',fontSize:13,color:'#4E4C4A'}}>
      ...
      <p style={{margin:'14px 0 0',color:'#4E4C4A'}}><strong>Scope limit:</strong> {s.scope}</p>
    </section>})}
    <section style={{borderTop:'1px solid #C9C6BF',padding:'28px 0'}}>
