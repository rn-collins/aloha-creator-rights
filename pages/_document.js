...
export default function Document() {
  ...
  window.addEventListener('load',function(){
    fetch('/api/track',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({event:'page_view',source:'aloha-creator-rights',referrer:document.referrer||'direct',utm:window._getUTM()})
    }}).catch(function(){});
  });
  ...
  window.addEventListener('scroll',function(){
    ...
    ms.forEach(function(m){if(pct>=m&&!fired[m]){fired[m]=1;
      fetch('/api/track',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({event:'scroll_depth',source:'aloha-creator-rights',depth:m+'%'})}}).catch(function(){});
    }});
  },{passive:true});
})()`}} />
      </Head>
